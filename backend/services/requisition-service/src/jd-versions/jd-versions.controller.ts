...
@ApiTags('HR-M1-FR-004 · JD versions')
...
export class JdVersionsController {
  ...
  @ApiOperation({ summary: 'Full version history (Approved / Superseded / Rejected included)' })
  @Get()
  ...
  @ApiOperation({ summary: 'Every edit is a new version (first = Draft; with based_on_version_id = Revising)' })
  @ApiBody({
    schema: {
      type: 'object',
      example: { content: 'Senior Data Analyst — you will own retail sales dashboards in Power BI…', variant_type: 'formal', based_on_version_id: null },
    },
  })
  @Post()
  ...
  @ApiOperation({ summary: 'Send a Draft/Revising version for approval (-> PendingApproval)' })
   ...
   */
  @ApiOperation({ summary: 'Always 409 version_immutable — versions are never edited in place' })
  @ApiBody({ schema: { type: 'object', example: { content: 'changed text' } } })
  @Patch(':versionId')
